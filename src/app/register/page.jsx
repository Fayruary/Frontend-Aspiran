 ...
 const handleRegister = async (e) => {
  ...
  try {
    ...
    setError(
      ...
      "Register gagal. Periksa koneksi backend."
    );
  } finally {
    setLoading(false);
  }
  ...
  return (
    <div className="min-h-screen bg-dark flex">
      ...
      <div className="hidden lg:flex w-1/2 flex-col justify-between p-12 bg-[#141414] border-r border-[#2A2A2A] relative overflow-hidden">
        ...
        <div className="absolute top-1/2 -right-24 w-48 h-48 rounded-full bg-gold/5" />
        ...
        <div className="z-10">
          <p className="font-display text-4xl text-cream leading-tight tracking-tight mb-6">
            Suara Anda<br />
            <em className="text-gold">menggerakkan</em><br />
            perubahan.
          </p>
          <p className="text-[#5C5850] text-sm leading-relaxed max-w-xs">
            Bergabung dengan ribuan warga yang aktif melaporkan masalah di sekitar mereka.
          </p>

          {/* Stats strip */}
          <div className="flex gap-8 mt-10">
            <div>
              <p className="font-display text-2xl text-gold">247+</p>
              <p className="text-[#5C5850] text-xs mt-0.5">Laporan masuk</p>
            </div>
            <div>
              <p className="font-display text-2xl text-[#4CAF7D]">76%</p>
              <p className="text-[#5C5850] text-xs mt-0.5">Diselesaikan</p>
            </div>
            <div>
              <p className="font-display text-2xl text-cream">3 hr</p>
              <p className="text-[#5C5850] text-xs mt-0.5">Rata-rata respons</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-8 md:px-16 lg:px-20 py-12">
        ...
        <div className="w-full max-w-sm mx-auto lg:mx-0">
          ...
          {/* Header */}
          <div className="mb-8">
            <p className="text-xs font-medium tracking-[2px] uppercase text-[#5C5850] mb-2">Langkah pertama</p>
            <h1 className="font-display text-3xl text-cream leading-tight tracking-tight">
              Buat akun<br />
              <em className="text-gold not-italic">gratis</em> sekarang
            </h1>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-3">
            ...
            {/* Username */}
            <div className="group">
              <label className="block text-xs text-[#5C5850] mb-1.5 ml-1">Username</label>
              <input
                type="text"
                name="username"
                placeholder="contoh: budi_jakarta"
                value={form.username}
            ...
            {/* Password */}
            <div>
              <label className="block text-xs text-[#5C5850] mb-1.5 ml-1">Password</label>
          ...
          <p className="text-center text-sm text-[#5C5850]">
            Sudah punya akun?{" "}
            <Link href="/login" className="text-gold hover:text-amber-300 transition-colors font-medium">
              Masuk di sini
            </Link>
          </p>

          {/* Terms */}
          <p className="text-center text-xs text-[#3A3A3A] mt-6 leading-relaxed">
            Dengan mendaftar, Anda menyetujui{" "}
            <Link href="/syarat" className="text-[#5C5850] hover:text-gold transition-colors underline underline-offset-2">Syarat & Ketentuan</Link>
            {" "}kami.
          </p>
